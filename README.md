...
The navigation sidebar can be collapsed to a compact icon-only strip by
clicking the chevron button on its right edge. Collapsed state is remembered
in browser local storage and is fully animated with a smooth CSS transition.
On mobile the sidebar becomes a full-height overlay triggered by the ☰ button;
the collapse control is automatically hidden on small screens.

## New: Enhanced System Logs

The **System logs** page now reads directly from **journald** via `journalctl`
(300 lines, all priorities). It falls back to classic syslog files
(`/var/log/syslog`, `/var/log/messages`) on systems without journald. The log
panel includes:

- **Filter chips** — All · Errors · Warnings · Kernel · Storage
- **Live search** — instant substring filter across all lines
- **Colour-coded output** — red for errors, amber for warnings, blue for kernel
  messages, green for storage events
- **Stats bar** — counts of errors, warnings, storage events and total lines

## New: Comprehensive ZFS Support

The Storage page shows full ZFS pool information including vdev topology
(mirror, RAIDZ-1/2/3, dRAID, stripe, spare), per-vdev health indicators, disk
type badges and temperature per member. Each ZFS pool card also loads its
**dataset tree** asynchronously from `/api/zfs/datasets`:

- Dataset and zvol listing with used / available figures
- Usage bar per dataset
- Compression ratio displayed where active
- Mountpoint or type label

ZFS scrub can be started and stopped from the pool card. Live scrub progress
with speed and ETA is polled every few seconds while a scrub is running.

## New: Hierarchical Storage & RAID Detection

The storage dashboard natively supports hierarchical display for ZFS pools,
Unraid arrays, Linux md-RAID, LVM, and Btrfs. Disks are automatically grouped
into their respective VDEVs or roles (`mirror`, `raidz`, `parity`, `data`,
`cache`, `spare`). md-RAID cards show the active/total drive count (e.g.
`3/4`) and a colour-coded RAID level badge (Mirror · RAID-5 · Linear · …).

## New: Disk Type Badges & Type-Aware Temperatures

Every disk in the temperature panel and storage member list now shows a type
badge — **NVMe**, **SSD** or **HDD**. Temperature colour thresholds are
applied per type:

| Type | Warm | Hot | Critical |
| --- | --- | --- | --- |
| NVMe | 40 °C | 75 °C | 85 °C |
| SATA SSD | 40 °C | 65 °C | 75 °C |
| HDD | 40 °C | 50 °C | 60 °C |
...
- Collapsible sidebar with smooth animation and localStorage state persistence
...
- **NVMe / SSD / HDD type badges** with type-aware temperature colour coding
- Scrollable overflow panels for temperature, container and storage member lists
- Flexible storage views for Unraid arrays, ZFS pools/VDEVs, Linux md-RAID,
  LVM, Btrfs and standalone Linux disks
- **ZFS dataset and zvol tree** per pool card with usage bars and compression ratio
- **ZFS scrub** start/stop with live progress, speed and ETA
- md-RAID active/degraded drive counts and RAID level badge
...
- System log stream from journald (300 lines) with filter chips, live search
  and colour-coded output (errors · warnings · kernel · storage)
- Host process view
...
Browser-only preferences such as language, sidebar state and card order remain
...
| `/dev:/host/dev` | Block device access for SMART and disk enumeration | Read-only |
| `/run/udev:/host/run/udev` | udev metadata for disk model and serial discovery | Read-only |
...
| ZFS | Pool capacity, health, VDEVs, physical members, dataset/zvol tree |
| Linux md-RAID | RAID level badge, active/total drive count, member devices, degraded state |
| LVM | Logical storage grouping, physical members and filesystem usage |
| Btrfs | Filesystem/pool, devices, capacity and health |
| Standalone disks | NVMe / SSD / HDD type badge, capacity, health and mount usage |
...
Temperature thresholds are applied per disk type:
...
The **System logs** page reads up to 300 lines from **journald** via
`journalctl --priority=debug` on modern hosts, and falls back to
`/var/log/syslog` or `/var/log/messages` on older systems. Logs are
colour-coded and filterable by category (All · Errors · Warnings · Kernel ·
Storage) with a live search field and a stats bar showing event counts.
...
host mount, `/dev:/host/dev:ro`, `/run/udev:/host/run/udev:ro` and
`/sys:/host/sys:ro`. Also check whether the host itself exposes SMART
information:
...
Virtual disks and some storage controllers do not expose temperatures. NVMe
drives report temperatures independently of SMART via the kernel's nvme driver
and are shown when the `/sys` tree is mounted correctly.

### Logs show "No classic host log file found"

On journald-only systems (most modern distributions), the log page reads from
journald via `journalctl`. If the dashboard container cannot reach the host
journal, ensure `/:/host` is mounted and `privileged: true` is set. The CLI
tab can always access live logs via `journalctl -f` directly.
