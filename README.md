...
- [Arcane installation](#arcane-installation)
...
- Outbound HTTPS access for GitHub version checks, registry update checks and
  optional Discord notifications; distribution logos are loaded by the browser
  from Simple Icons when available
...
ZFS pool cards include VDEV topology, per-member health and temperature,
datasets and zvols with capacity and compression information, and scrub
start/stop controls with live progress. md-RAID cards show the RAID level and
active/total member count. Devices are grouped by role, including mirror,
RAIDZ, parity, data, cache and spare.
...
After verifying the updated container, unused images can optionally be removed
...
the multi-stage build instead of being loaded from a CDN at runtime.

| Path | Purpose |
| --- | --- |
| `server.py` | HTTP/WebSocket server, authentication and host integrations |
| `web/` | Browser interface, styles, application logic and translations |
| `compose.yml` | Production deployment definition |
| `Dockerfile` | Multi-stage AMD64/ARM64 image build |
| `docs/screenshots/` | Generated demonstration screenshots |
